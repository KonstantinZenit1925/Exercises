package MapHashMapLinkedHashMap;
/**
 * Начнем знакомиться с методами, которые определены в интерфейсе Map<K, V>,
 * который представляет собой отображение, в котором каждому значению K (ключ) соответствует значение V(value).
 * Map используется очень часто, поэтому тему нужно освоить очень хорошо.
 *
 * Начнем с методов, которые позволяют добавить новый элемент в коллекцию,
 * а также проверить существует ли уже такой ключ в Map:
 *
 ...
 * Если ключа еще не было в коллекции, то метод возвращает null,
...
public class PutContainsKey {
    // Класс выполняет вставку значений в отображение без проверки содержится ли такой ключ уже в отображении.
    public static Map<Integer, User> addNewElementWithoutCheck(List<User> list) {
        Map<Integer, User> rsl = new HashMap<>();
        for (User user : list) {  // проходим циклом фоич по нашему списку, и кладем в него с помощью
            rsl.put(user.id, user); // метода put вставку значения, ключ id, значение объект типа User
        }
        return rsl;
    }
    // Класс ыполняет вставку значений в отображение с проверкой содержится ли такой ключ уже в отображении.
    public static Map<Integer, User> addNewElementWithCheck(List<User> list) {
        Map<Integer, User> rsl = new HashMap<>();
        for (User user: list) {
            rsl.put(user.id, user);  // вставляем значение
            rsl.containsKey(user.getId()); // выполняем проверку содержится ли такое значение с помощью метода
        }
        return rsl;
    }
