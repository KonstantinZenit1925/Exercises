...
/**
 * В интерфейсе List<E> определен особый метод listIterator(), который возвращает объект ListIterator<E>
 ...
 *
 * Вы скажете - у нас уже есть Iterator<E>, для чего нам нужен еще один? Ответ прост - названия у них похожи,
 * но работают они по-разному, поэтому укажем важные различия и каким пользоваться - выбор за Вами.
 *
 ...
 *
 * 3. ListIterator допускает модификацию списка, за счет его дополнительных методов add и remove.
 * Iterator<E> такой возможности не имеет.
 *
 * В дополнение к методам, которые были определены в Iterator<E>, в ListIterator добавлены еще несколько:
 *
