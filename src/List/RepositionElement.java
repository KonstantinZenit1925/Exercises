package List;
/**
 * Продолжим модифицировать список, познакомимся с методами для удаления элементов в списке
 * и замены элемента по индексу. Для этого предназначены методы set() и remove():
 ...
 * а потом этим элементом заменить элемент по индексу, который пришел в параметрах метода. Необходимо учесть,
...
public class RepositionElement {
    public static List<String> changePosition(List<String> list, int index) {
        String indexOfLastElement = list.remove(list.size() -1);
        if (index < list.size()) {
            list.set(index, indexOfLastElement);
        }
        return list;
    }
}
