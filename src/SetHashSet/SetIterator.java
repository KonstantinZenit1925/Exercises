 ...
 * который позволяет напрямую получить значение из множества (по аналогии с List,
 * у Set нет метода get()). Что же делать? Ведь нам же как-то надо получать значения.
 * Для этого можно использовать Iterator, который работает по тому же принципу
 * как у List (поскольку оба интерфейса наследуются от Collection).
 *
 * Iterator<E> iterator() - метод возвращает объект Iterator,
 * который позволяет обойти весь набор значений.
 *
