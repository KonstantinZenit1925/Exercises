package SetHashSet;
/**
 * Познакомимся с интерфейсом Set и его реализациями. Он расширяет интерфейс Collection и
 * представляет собой набор уникальных элементов, т.е. в наборе значений не могут находиться одинаковые значения.
 ...
 *
 * Set не добавляет никаких методов по сравнению с Collection, только вносит изменения в унаследованные.
 * Например метод add():
 *
 * boolean add​(E e) - добавляет новый элемент e в коллекцию. метод возвращает true только в том случае,
 * если такого элемента еще нет в коллекции.
 *
